...
- 论坛出现 ESA 普通 JS 验证时，自动复用 MoviePilot 无头浏览器正常访问；遇人工滑块会停止并通知，不会误报 Cookie 失效或持续重试
...
- **输出用户信息一次**: 保存配置后读取个人资料与密码安全页，将用户名、UID、邮箱和手机号输出到日志，执行后自动关闭
- **最大重试次数**: 签到失败时的重试次数
- **重试间隔(秒)**: 每次重试的等待时间
- **历史保留天数**: 签到历史记录的保留时间
...
- 用户信息会以明文写入MoviePilot日志，仅在确实需要找回账号信息时使用一次性开关
- 如签到失败，请检查Cookie是否有效，可尝试重新获取；若通知提示“人工滑块验证”，请先在浏览器完成验证后再试，插件不会尝试绕过验证

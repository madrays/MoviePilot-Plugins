...
- 使用“隐秘的角落”生成的 MP 专用 API Key 接入，无论坛账号、密码、Cookie 或页面抓取
- 自动完成蜂巢签到，并在插件内展示论坛头像、勋章、连签摘要与最近签到记录
- 从 MoviePilot 本地统计模型同步 PT 人生；失败使用单一非阻塞延迟任务并复用同一幂等批次，不访问任何 PT 站点
- 定时任务自动随机错峰，插件详情页只读本地缓存，不因打开页面请求论坛
